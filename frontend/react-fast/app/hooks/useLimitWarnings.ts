      ...
      try {
        ...
        if (usage.warnings?.approaching_limit) {
          if (tier === 'free') {
            ...
            showOnce('approaching_limit_free', `⚠️ Only ${remaining} messages left today!`, 'warning', 5000);
          } else if (tier === 'starter') {
            const remaining = (usage.limits?.requests_per_month || 500) - (usage.prompt_count || 0);
            showOnce('approaching_limit_starter', `⚠️ Only ${remaining} monthly requests left!`, 'warning', 5000);
          } else if (tier === 'pro') {
            const remaining = (usage.limits?.requests_per_month || 2000) - (usage.prompt_count || 0);
            showOnce('approaching_limit_pro', `⚠️ Only ${remaining} monthly requests left!`, 'warning', 5000);
          }
        }
